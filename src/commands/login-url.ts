...
/** Watches a login's output for the first sign-in URL and acts on it once.
 *
 * Once, deliberately: vendors reprint the URL (on a redraw, on a retry), and
 * copying it again mid-paste would clobber the code the user is about to
 * paste back. Output arrives in arbitrary chunks, so a URL is only acted on
 * after the line holding it is complete -- otherwise a chunk boundary yields
 * half a link, silently. */
export class LoginUrlWatcher {
  private buffer = '';
  ...
  /** True when this chunk completed a sign-in URL and it was acted on. */
  push(chunk: string): boolean {
    if (this.handled) return false;
    this.buffer += chunk;
    // Keep only the newest text: a login prints little, but a vendor TUI that
    // redraws can emit a great deal, and this must not grow without bound.
    ...
    const url = extractLoginUrl(this.buffer.slice(0, lastBreak));
    if (!url) return false;
    this.handled = true;
    const environment = this.options.environment ?? process.env;
    const notice = loginUrlNotice(url, environment);
    if (hasLocalDisplay(environment)) (this.options.open ?? openLoginUrl)(url);
    this.options.write(`${notice.clipboard}\n${notice.lines.join('\n')}\n`);
    return true;
  }

  get seenUrl(): boolean { return this.handled; }
}
