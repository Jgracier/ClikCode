...
/** `auth application-default login` alone only writes the ADC file --
 * confirmed against gcloud's own documented distinction: it does NOT set
 * the active account gcloud's own commands (like `projects create` right
 * after this) need, which is a separate credential store entirely. `auth
 * login --update-adc` does both from one OAuth consent: sets the active
 * account *and* writes the same credentials to the ADC location, "any
 * credentials previously generated by gcloud auth application-default
 * login" included -- one browser flow instead of two. */
async function runGcloudApplicationDefaultLogin(gcloudBinary: string, homePath: string): Promise<void> {
  await new Promise<void>((resolve, reject) => {
    const child = spawn(gcloudBinary, ['auth', 'login', '--update-adc'], {
      stdio: 'inherit',
    ...
    child.once('exit', (code) => {
      if (code === 0) resolve();
      else reject(new Error(`gcloud auth login exited ${code}`));
    });
  });
}

/** Captured, not inherited: these two run after the interactive OAuth step
 * is already done, so there's nothing left for a user to see or respond to
 * -- capturing stderr means a real failure (a ToS wall, an org policy
 * restriction, anything project-creation-specific for a brand-new account)
 * surfaces as gcloud's own actual message, not a generic wrapped error. */
async function runGcloudCaptured(gcloudBinary: string, args: readonly string[], homePath: string): Promise<string> {
  return new Promise<string>((resolve, reject) => {
    const child = spawn(gcloudBinary, [...args, '--quiet'], {
    ...
    child.once('exit', (code) => {
      ...
      else reject(new Error(`gcloud ${args.join(' ')} failed: ${(stderr || stdout).trim() || `exited ${code}`}`));
    });
  });
}

/** A quota project is required for ADC to work at all -- confirmed against
 * Google's own ADC troubleshooting docs: `gcloud auth application-default
 * login` alone authenticates using gcloud's own internal OAuth client, and
 * an app checking for a real quota project (which Antigravity does --
 * this is what "agy exited 3" traced back to) fails without one. Creating
 * a small, dedicated project per account and setting it as the quota
 * project keeps this fully automatic -- no GCP Console, no pre-existing
 * project required from the user, matching every other harness's "just
 * works" account setup. Project ID must start with a letter and stay
 * within 30 characters (a real GCP constraint, not arbitrary); accountId
 * is a UUID, so prefixing with a letter and truncating covers both.
 */
async function ensureAntigravityQuotaProject(gcloudBinary: string, homePath: string, accountId: string): Promise<void> {
  const projectId = `clikcode-${accountId.replace(/-/g, '').slice(0, 20)}`;
  await runGcloudCaptured(gcloudBinary, ['projects', 'create', projectId, '--name=ClikCode Antigravity'], homePath);
  await runGcloudCaptured(gcloudBinary, ['auth', 'application-default', 'set-quota-project', projectId], homePath);
}
...
export async function aiAccountLogin(harnessCommandName: string, label?: string): Promise<string> {
  ...
  if (harness.command === 'antigravity' && profilePath) {
    ...
    output.write(`\n${chalk.dim('Finishing setup (creating a dedicated Google Cloud project for this account)…')}\n\n`);
    await ensureAntigravityQuotaProject(gcloudBinary, profilePath, accountId);
