...
const OPENCODE_FAMILY = {
  ...
  localAuth: ['api-key', 'oauth', 'vendor-cli'], loginArgv: ['auth', 'login'], providerLoginArgv: ['auth', 'login', '--provider', '{provider}'], modelProviderSeparator: '/', modelArgvPrefix: ['--model'], modelDiscoveryArgv: ['models'], workspaceArgvPrefix: ['--dir'],
...
export const AI_LOCAL_HARNESSES: readonly AiLocalHarnessDefinition[] = [
  ...
  { command: 'kiro', provider: 'kiro', displayName: 'Kiro CLI', surface: 'terminal', tier: 'more', transport: 'structured-cli', integration: 'structured', parser: 'generic-json', memoryFile: 'AGENTS.md', nativeSlashPassthrough: false, acp: { argv: ['acp'], experimental: true }, effortValues: ['low', 'medium', 'high', 'xhigh', 'max'], localAuth: ['oauth', 'vendor-cli'], loginArgv: ['login'], logoutArgv: ['logout'], statusArgv: ['whoami'], binary: 'kiro-cli', launchArgv: ['chat'], modelArgvPrefix: ['--model'], effortArgvPrefix: ['--effort'], permissionModes: ['ask', 'bypass'], permissionArgv: { ask: { argv: [] }, bypass: { argv: ['--trust-all-tools'] } }, turn: { promptGuard: 'double-dash', startArgv: ['chat', '--no-interactive', '--agent-engine', 'v3', '--output-format', 'stream-json'], resumeIdPrefix: ['--resume-id'], output: 'json-lines', responseFields: ['text', 'content', 'result'] }, session: { resumeIdPrefix: ['chat', '--resume-id'], continueArgv: ['chat', '--resume'], discoverArgv: ['chat', '--list-sessions'], discoverFormat: 'text' } },
