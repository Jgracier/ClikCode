...
export interface AiHarnessAcpDefinition {
  ...
  /** The ACP `session/new` result lists the models (`availableModels`) and
   * the vendor has no command that does. Only where opening a session to ask
   * leaves nothing in the vendor's history (checked for Cline 3.0). */
  listsModels?: boolean;
}
...
export const AI_LOCAL_HARNESSES: readonly AiLocalHarnessDefinition[] = [
  ...
  { command: 'vibe', provider: 'mistral-vibe', displayName: 'Mistral Vibe', surface: 'terminal', tier: 'more', transport: 'acp', integration: 'structured', parser: 'generic-json', memoryFile: 'AGENTS.md', nativeSlashPassthrough: false, acp: { binary: 'vibe-acp', argv: [] }, experimental: true, localAuth: ['api-key', 'vendor-cli'], binary: 'vibe', loginArgv: ['--setup'], authFiles: [{ path: '${VIBE_HOME:-~/.vibe}/.env', contains: 'MISTRAL_API_KEY=', removeLine: true }], authEnv: ['MISTRAL_API_KEY'], workspaceArgvPrefix: ['--workdir'], permissionModes: ['ask', 'bypass', 'auto'], permissionArgv: { ask: { argv: ['--agent', 'ask'] }, bypass: { argv: ['--auto-approve'] }, auto: { argv: ['--smart-approve'] } }, turn: { startArgv: ['--output', 'streaming'], promptArgvPrefix: ['--prompt'], output: 'json-lines', responseFields: ['text', 'content', 'response', 'result'] }, session: { resumeIdPrefix: ['--resume'], continueArgv: ['-c'] } },
